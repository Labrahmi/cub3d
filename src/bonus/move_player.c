...
/*   Updated: 2023/10/04 11:40:10 by ayakoubi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	get_cord_to_move(t_data *data, int *new_x, int *new_y, int key)
{
	int	dx;
	int	dy;

	dx = data->player.move_speed * cos(data->player.rot_ang * (M_PI / 180.0));
	dy = data->player.move_speed * sin(data->player.rot_ang * (M_PI / 180.0));
	if (key == MLX_KEY_W)
	{
...
void	draw_the_hook(t_data *data, int new_x, int new_y)
{
	if (is_valid_position(data, new_x, new_y)
		&& is_valid_position(data, data->player.x, new_y)
		&& is_valid_position(data, new_x, data->player.y))
	{
		data->player.x = new_x;
		data->player.y = new_y;
	}
